...
ipcMain.handle('run-stt', async (event, { model, audioPath, language }) => {
    return new Promise((resolve, reject) => {
        ...
        const basePath = app.isPackaged ? process.resourcesPath : __dirname;
        const whisperExe = path.join(basePath, 'engine', 'cpu_avx2', 'whisper.exe');
        if (!fs.existsSync(whisperExe)) {
            reject(new Error(`Whisper executable not found at ${whisperExe}`));
            return;
        }
        if (!fs.existsSync(modelPath)) {
            reject(new Error(`STT model not found: ${model}`));
            return;
        }

        const ffmpegDir = path.join(basePath, 'engine', 'cpu_avx2');
        ...
        (async () => {
            try {
                ...
                const transcript = await transcribeWithWhisperServer(port, audioPath);
